    ...
    for f in SRC.iterdir():
        ...
        if "patient_extract" in n and ("filtered" in n or "corrected" in n or "1602" in n):
            if diary_f is None or "filtered" in n or "corrected" in n:
                diary_f = f.name
        if "myfood24" in n and "matched" in n:
            mapping_f = f.name
    if diary_f is None:
        diary_f = "patient_extract1602_filtered.csv"
    if mapping_f is None:
        mapping_f = "MyFood24 ID Matched(Sheet1).csv"
    return diary_f, mapping_f


def _persist_pipeline_inputs(diary_filename: str, mapping_filename: str):
    payload = {"diary_file": diary_filename, "mapping_file": mapping_filename}
...
def _get_tz_offset(diary_sub):
    ...
    for s in raw:
        try:
            ...
            pts = off[1:].split(":")
            td = timedelta(hours=sign * int(pts[0]), minutes=sign * int(pts[1]) if len(pts) > 1 else 0)
            offsets.append(td)
        except Exception:
            pass
    if offsets:
...
def step1():
    ...
    print("=" * 72)
    print("STEP 1: Load ID Mapping and Discover CGM Files")
    print("=" * 72)
...
def step2(diary, mapping):
    print("\n" + "=" * 72)
    print("STEP 2: Classify Data Quality")
    print("=" * 72)
    time_fmts = {}
    day_info = {}
    p12 = []
...
def step3(cgm_path, diary_sub):
    ...
    for j in range(len(bounds) - 1):
        ...
        if nv >= SMOOTHING_WINDOW:
            sv = savgol_filter(v[ok], SMOOTHING_WINDOW, SMOOTHING_POLY)
...
def _detect_day(cg, mf24, ctr):
    ...
    i = 0
    while i < n:
        if np.isnan(dg[i]) or dg[i] <= DGDT_THRESHOLD:
            i += 1
            continue
        rs = i
        while i < n and not np.isnan(dg[i]) and dg[i] > DGDT_THRESHOLD:
            i += 1
        re = i - 1
        if re - rs + 1 < MIN_SUSTAINED_READINGS:
            continue
        g0, g1 = gl[rs], gl[re]
        if np.isnan(g0) or np.isnan(g1):
            continue
        if g1 - g0 < MIN_RISE_IN_WINDOW_MMOL:
            continue
    ...
    for rs, re in candidates:
        ...
        pe = min(n, ps + 18)
        seg_p = gl[ps:pe]
        if np.all(np.isnan(seg_p)):
            continue
        pi = ps + int(np.nanargmax(seg_p))
        if np.isnan(gl[ni]) or np.isnan(gl[pi]):
            continue
        rise = gl[pi] - gl[ni]
        if rise < MIN_EXCURSION_RISE_MMOL:
            continue
        ctr[0] += 1
        excs.append(
            {
        ...
        for e in excs[1:]:
            ...
            if gap < EXCURSION_MERGE_MIN:
                if e["peak_glucose"] > p["peak_glucose"]:
                    p["peak_time"] = e["peak_time"]
                    p["peak_glucose"] = e["peak_glucose"]
...
def _agg(rows, label, d):
    ...
    for col in NUTRIENT_COLS:
        result[f"total_{col}"] = (
            pd.to_numeric(df[col], errors="coerce").sum() if col in df.columns else 0.0
        )
    return result
...
def step6(bundles_by_date, exc_by_date, tf):
    for d, bundles in bundles_by_date.items():
        ...
        for b in bundles:
            if tf != "12h":
                b["ampm_resolved_by"] = "24h_format"
                continue
            rt = b["reported_time"]
            if not rt:
                b["ampm_resolved_by"] = "fallback"
                continue
            h = rt.hour
            if h == 0 or h == 12:
                b["ampm_resolved_by"] = "unambiguous"
                continue
            t_am, t_pm = rt, rt.replace(hour=h + 12)
            if day_exc and b["total_CHO"] > CHO_THRESHOLD:
                sa = _ampm_sc(t_am, day_exc)
                sp = _ampm_sc(t_pm, day_exc)
                if sa > 0 or sp > 0:
                    if sp > sa:
                        b["reported_time"] = t_pm
                    b["ampm_resolved_by"] = "cgm"
                    continue
...
def step8(bundles_by_date, exc_by_date, day_info, tf, mf24, gaps, cgm_dates):
    ...
    for d, bundles in sorted(bundles_by_date.items()):
        ...
        day_exc = exc_by_date.get(d, [])
        ext_exc = (
            [e for e in exc_by_date.get(d - timedelta(days=1), []) if e["est_meal_time"].hour >= 20] + day_exc
            if not is_batch
            else list(day_exc)
        )
        ...
        for i, b in enumerate(bundles):
            ...
            r.update(
                {
                    "reported_time": b["reported_time"],
                    "corrected_time": b["reported_time"],
                    "time_shift_min": 0.0,
                    "confidence": "",
                    "match_type": "",
                    "excursion_id": "",
                    "stacked_onto_event_id": "",
                    "excursion_rise_mmol": np.nan,
                    "excursion_peak_mmol": np.nan,
                    "nadir_time": pd.NaT,
                    "peak_time": pd.NaT,
                    "batch_day": is_batch,
                    "time_format": tf,
            ...
            if b["total_CHO"] <= CHO_THRESHOLD:
                r["confidence"] = "low_cho_no_match"
            elif not has_cgm:
                r["confidence"] = "no_cgm_data"
            elif is_flat:
                r["confidence"] = "flat_trace"
            elif in_gap(b["reported_time"]):
                r["confidence"] = "cgm_gap"
            elif i in anchored:
                exc, _ = anchored[i]
                ...
                ai = stacked[i]
                exc = anchored[ai][0]
                r["confidence"] = "stacked"
                r["match_type"] = "stacked"
                ...
                r["nadir_time"] = exc["nadir_time"]
                r["peak_time"] = exc["peak_time"]
            else:
...
def _batch_assign(sig, day_exc, anchored, assigned):
    n_sig, n_exc = len(sig), len(day_exc)
    if n_exc == 0 or n_sig == 0:
        return
    sig_s = sorted(sig, key=lambda x: x[1]["reported_time"] or datetime.min)
    if n_exc >= n_sig:
        sel = sorted(sorted(day_exc, key=lambda e: e["rise_mmol"], reverse=True)[:n_sig], key=lambda e: e["est_meal_time"])
    else:
        by_sug = sorted(sig_s, key=lambda x: x[1]["total_TOTSUG"], reverse=True)
        sig_s = sorted(by_sug[:n_exc], key=lambda x: x[1]["reported_time"] or datetime.min)
        sel = sorted(day_exc, key=lambda e: e["est_meal_time"])
    for k in range(min(len(sig_s), len(sel))):
        bi, b = sig_s[k]
        exc = sel[k]
        b["_corr"] = exc["est_meal_time"]
        anchored[bi] = (exc, None)
        assigned.add(exc["excursion_id"])


def _rt_assign(sig, excs, anchored, assigned):
    if not excs or not sig:
        return
    ...
    for exc in sorted(excs, key=lambda e: e["est_meal_time"]):
        ...
        for bi, b in sig:
            if bi not in avail or not b["reported_time"]:
                continue
            sh = abs((b["reported_time"] - exc["est_meal_time"]).total_seconds() / 60)
            if sh > MAX_ALLOWABLE_SHIFT_MIN:
                continue
            tc = W_TIME * sh / 60
            ns = b["total_TOTSUG"] / sug_max
            nr = exc["rise_mmol"] / rise_max
            c = tc + (W_SUG * abs(ns - nr))
            if c < best_c:
                best_c, best_i = c, bi
...
def _apply(r, b, exc, is_batch, tf):
    rep = b["reported_time"]
    est = exc["est_meal_time"]
    if not rep or not est:
        r["confidence"] = "no_match"
        return
    ...
    if a > MAX_ALLOWABLE_SHIFT_MIN:
        if is_batch:
            r["confidence"] = "low_batch_override"
            r["corrected_time"] = est
            r["time_shift_min"] = round(raw, 1)
            return
        if tf == "12h" and ampm == "cgm":
            r["confidence"] = "low_ampm_override"
            r["corrected_time"] = est
            r["time_shift_min"] = round(raw, 1)
            return
        r["confidence"] = "rejected_implausible_shift"
        return
    if a < 30:
        r["confidence"] = "high"
        r["corrected_time"] = est
    ...
    r["confidence"] = "low_clamped"
    r["corrected_time"] = rep + timedelta(minutes=cl)
    r["time_shift_min"] = round(cl, 1)
        ...
        rows.append(
            {
                ...
                "matched_high": confs.get("high", 0),
                "matched_medium": confs.get("medium", 0),
...
def step10_plot(all_results, cgm_cache, mapping):
    ...
    pids_done = set()
    for mf24, (pid, cgm_p) in mapping.items():
        if pid in pids_done or cgm_p is None:
            continue
        pids_done.add(pid)
        pr = [r for r in all_results if r["myfood24_id"] == mf24]
        if not pr:
        ...
        for di, d in enumerate(all_dates):
            ...
            ymin, ymax = ax.get_ylim() if len(dc) > 0 else (4, 10)
            if len(dc) > 0:
                gs = dc["glucose"].dropna()
                if len(gs) > 0:
                    ymin, ymax = gs.min(), gs.max()
            arrow_idx = 0
            label_abbr = {"Breakfast": "B", "Lunch": "L", "Evening dinner": "D", "Snack": "S", "Drink": "Dr"}
            for r in day_r:
                rt = r["reported_time"]
                ct = r["corrected_time"]
                ml = r.get("meal_label", "")
                if rt:
                    ...
                    abbr = label_abbr.get(ml, ml[:2] if ml else "")
                    ax.text(
                        mdates.date2num(rt),
                        ymin - (ymax - ymin) * 0.02,
                        abbr,
                ...
                if ct and rt and ct != rt:
                    ...
                    ax.text(
                        mdates.date2num(rt) + (mdates.date2num(ct) - mdates.date2num(rt)) / 2,
...
def step10_global(all_results):
    ...
    axes[0, 0].set_title("Time shift distribution")
    axes[0, 0].set_xlabel("Shift (min)")
    ...
    conf_colors = {
        ...
        "medium": "orange",
        "low_clamped": "red",
        "low_batch_override": "purple",
        "low_ampm_override": "magenta",
        "stacked": "cyan",
    }
    for c, clr in conf_colors.items():
        sub = matched[matched["confidence"] == c]
        if len(sub):
            axes[0, 1].scatter(sub["total_CHO"], sub["excursion_rise_mmol"], c=clr, s=8, alpha=0.5, label=c)
    axes[0, 1].set_title("CHO vs Excursion Rise")
    axes[0, 1].set_xlabel("CHO (g)")
    axes[0, 1].set_ylabel("Rise (mmol/L)")
    axes[0, 1].legend(fontsize=6)

    counts = df["confidence"].value_counts()
    axes[1, 0].pie(counts, labels=counts.index, autopct="%1.1f%%", textprops={"fontsize": 7})
    ...
    if len(matched2) > 0:
        ...
        axes[1, 1].set_title("Mean shift by meal type")
        axes[1, 1].set_ylabel("Shift (min)")
...
def main():
    ...
    for idx, mf24 in enumerate(mf24_list):
        ...
        if cgm_path:
            ...
            cgm_cache[mf24] = cgm
        exc_by_date = step4(cgm, mf24) if cgm is not None else {}
        ...
        print(
            f"  [{idx+1}/{total}] MF24={mf24} PID={pid:>5s}  CGM  "
            f"exc={n_exc:>3d}  evts={len(results):>3d}  anc={n_anc:>3d}  stk={n_stk:>2d}  batch_d={n_batch}"
        )
        all_results.extend(results)
    print(f"\n  Total: {len(all_results)} meal events")
    print("\n" + "=" * 72)
    print("STEP 10: Produce Outputs")
    print("=" * 72)
